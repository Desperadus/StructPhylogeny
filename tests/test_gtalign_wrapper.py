...
def test_main_raises_when_no_gtalign_hits_match_manifest(tmp_path, monkeypatch):
    config_path = tmp_path / "config.yaml"
    manifest_path = tmp_path / "manifest.tsv"
    output_path = tmp_path / "pairwise.tsv"
    raw_dir = output_path.parent / "gtalign_raw"
    ...
    with __import__("pytest").raises(RuntimeError, match="none matched the manifest sample names"):
        module.main()
